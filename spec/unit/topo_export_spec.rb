...
describe KnifeTopo::TopoExport do
  ...
  describe "#run" do
    it "exports a template for an unknown topology" do
      @cmd  = KnifeTopo::TopoExport.new(['topo', 'export', '--min-priority=default'])
    ...
    it "exports a template for a defined topology" do
      @cmd  = KnifeTopo::TopoExport.new(['topo', 'export', '--topo=test1', '--min-priority=default'])
      item = Chef::DataBagItem.new
      item.raw_data = { 'id' => 'test1', 'name' => 'test1', 'nodes' => [] }
      allow(@cmd).to receive(:load_topo_from_server).with('test1').and_return(item)
      @cmd.run
      output = @cmd.load_or_initialize_topo
    ...
    it "exports a node for a new topology" do
      @cmd  = KnifeTopo::TopoExport.new(['topo', 'export', 'appserver1', '--min-priority=default'])
      allow(@cmd).to receive(:load_topo_from_server).and_return(nil)
      node = Chef::Node.new
